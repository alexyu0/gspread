...
class Spreadsheet(object):
    ...
    def batch_update(self, body, batch_requests=False, flush_requests=False):
        ...
        """
        if not batch_requests or flush_requests:
            send_request = True
            if len(self.batched_requests) > 0:
                body = {'requests': self.batched_requests + body['requests']}
        else:
            send_request = False
            self.batched_requests.extend(body['requests'])
            if len(self.batched_requests) >= 10:
                send_request = True
